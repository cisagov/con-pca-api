...
def start_subscription(data=None, subscription_uuid=None):
    ...
    deal_with_sending_profiles(subscription)  
...
def deal_with_sending_profiles(subscription):
    ...
    if sending_profile_name.endswith(subscription['name']):
        ...
        sending_profile_to_clone = next(iter([p for p in sending_profiles if p.name == sending_profile_name]), None)        
    else:            
        sending_profile_to_clone = next(iter([p for p in sending_profiles if p.name == sending_profile_name]), None)
        #check to make sure it is really not there                    
        sending_profile_name = sending_profile_name + "_" + subscription['name']        
        sending_profile_to_new_name = next(iter([p for p in sending_profiles if p.name == sending_profile_name]), None)
    ...
    if not sending_profile_to_clone.headers:
        ...
        header = next(iter([h for h in sending_profile_to_clone.headers if h['key'] == 'DHS-PHISH']),None)
