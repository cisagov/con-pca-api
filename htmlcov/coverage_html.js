      ...
      if (filter_value === "") {
        ...
        if (hidden > 0) {
          ...
          for (var column = 2; column < 20; column++) {
            ...
            var footer_cell = table_dynamic_footer.find(
              "td:nth-child(" + column + ")"
            );
...
coverage.selection_ends_on_screen = function () {
  ...
  return (top.isOnScreen() ? 1 : 0) + (next.isOnScreen() ? 1 : 0);
};
...
coverage.to_prev_chunk_nicely = function () {
  ...
  if (coverage.selection_ends_on_screen() === 0) {
    ...
    coverage.select_line_or_chunk(
      coverage.line_at_pos(win.scrollTop() + win.height())
    );
  }
  ...
  c.lines_to_mark.each(function () {
    ...
    if (line_number === previous_line + 1) {
      ...
      scroll_marker.append(
        '<div id="m' + line_number + '" class="marker"></div>'
      );
